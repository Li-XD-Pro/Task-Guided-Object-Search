...
def run_episode(player, args, total_reward, model_options, training):
    ...
    for _ in range(num_steps):
        ...
        print(f"Step {_+1}: Action taken, Reward: {player.reward}, Total Reward: {total_reward}")
...
def new_episode(
    ...
    # 调试输出：表示新的 episode 开始
    print("=== New Episode Started ===")
    print("Scene:", scenes)
    print("Possible targets:", possible_targets)
    # print("Target objects:", targets)
    print("Room:", rooms)
    ...
    for param, shared_param in zip(
        ...
        if shared_param.requires_grad:
            if param.grad is None:
                shared_param._grad = torch.zeros_like(shared_param)
            elif gpu_id < 0:
